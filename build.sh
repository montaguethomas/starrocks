...
if [ ${BUILD_BE} -eq 1 ]; then
    ...
    if [ "${BUILD_JAVA_EXT}" == "ON" ]; then
        ...
        cp -r -p ${STARROCKS_HOME}/java-extensions/hudi-reader/target/starrocks-hudi-reader-jar-with-dependencies.jar ${STARROCKS_OUTPUT}/be/lib
        ...
        cp -r -p ${STARROCKS_HOME}/java-extensions/iceberg-metadata-reader/target/starrocks-iceberg-metadata-reader-jar-with-dependencies.jar ${STARROCKS_OUTPUT}/be/lib/jni-packages
        cp -r -p ${STARROCKS_HOME}/java-extensions/iceberg-metadata-reader/target/starrocks-iceberg-metadata-reader-jar-with-dependencies.jar ${STARROCKS_OUTPUT}/be/lib/iceberg-reader-lib
        ...
        cp -r -p ${STARROCKS_HOME}/java-extensions/kudu-reader/target/starrocks-kudu-reader-jar-with-dependencies.jar ${STARROCKS_OUTPUT}/be/lib/jni-packages
        cp -r -p ${STARROCKS_HOME}/java-extensions/kudu-reader/target/starrocks-kudu-reader-jar-with-dependencies.jar ${STARROCKS_OUTPUT}/be/lib/kudu-reader-lib
