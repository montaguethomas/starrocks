...
RUN yum update -y && yum install -y java-1.8.0-openjdk-devel tzdata openssl curl vim ca-certificates fontconfig gzip tar less hostname procps-ng lsof nc && \
...
ARG USER=starrocks
ARG GROUP=starrocks
RUN groupadd --gid 1001 $GROUP && useradd --no-create-home --uid 1001 --gid 1001 \
